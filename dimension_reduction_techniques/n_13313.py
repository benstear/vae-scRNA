...
def plot2D(X,y,method,title,time_elapsed):
    ...
    fig.savefig('/scr1/users/stearb/results/plots_13313/'+method+str(np.float16(time_elapsed))+'.png')
...
plot2D(lda_array,y,'Isomap','Isomap: 13,313 cells with 49 cell subtypes',time_elapsed)
...
time_elapsed = time.time() - t
plot2D(lda_array,y,'Spectral','Spectral Embedding: 13,313 cells with 49 cell subtypes',time_elapsed)
...
time_elapsed = time.time() - t
plot2D(lda_array,y,'NNMF','Non-Negative Matrix Factorization: \n13,313 cells with 49 cell subtypes',time_elapsed)
